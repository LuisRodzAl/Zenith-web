...
try:
    if app.config['FIREBASE_CREDENTIALS']:
        ...
        print("⚠ Warning: FIREBASE_CREDENTIALS not set. Firebase features will be disabled.")
...
def check_firebase():
    if not db:
        return jsonify({'error': 'Firebase not configured. Please set FIREBASE_CREDENTIALS in .env'}), 503
    return None
...
def get_chat_history():
    ...
    messages_ref = db.collection('conversaciones').document(user_id).collection('mensajes')
...
def send_message():
    ...
    if not model:
        return jsonify({'error': 'Gemini AI not configured. Please set GEMINI_API_KEY in .env'}), 503
    ...
    db.collection('conversaciones').document(user_id).collection('mensajes').add(user_msg.to_dict())
    ...
    try:
        ...
        db.collection('conversaciones').document(user_id).collection('mensajes').add(ai_msg.to_dict())
...
def create_psychologist():
    ...
    try:
        ...
        return jsonify({'id': psicologo_id, 'message': 'Psychologist created successfully'}), 201
